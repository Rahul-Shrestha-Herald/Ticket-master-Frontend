...
const ManageSchedules = () => {
  ...
  const openModalForEdit = (schedule) => {
    // No date restriction: operator can edit any schedule (past or future)
                      ...
                      <ul className="list-disc ml-4 max-h-[150px] overflow-y-auto">
                        {formData.scheduleDates.map((date, idx) => (
                            <li key={idx} className="flex items-center">
                              <span>{date}</span>
                              <IconButton size="small" onClick={() => removeDate(date)} className="ml-1" aria-label="Remove date">
                    ...
                    <>
                      ...
                      <Box className="mb-4 max-h-[300px] overflow-y-auto">
                        {futureDates.length > 0 ? (
                          ...
                          <Typography variant="body1" className="text-sm sm:text-base">
                            This schedule only contains past dates. You can still delete the entire schedule.
                          </Typography>
                        )}
                      </Box>
                      {(
                            ...
                            onClick={async () => {
                              try {
                                if (futureDates.length > 0 && pastDates.length > 0) {
                                  // Has both: remove only future dates, keep past
                                  ...
                                  await axios.put(
                                    `${backendUrl}/api/operator/schedules/${deleteConfirmSchedule._id}`,
                                    {
                                      ...
                                      seats: { dates: updatedSeats }
                                    },
                                    { headers: { Authorization: `Bearer ${operatorData?.token}` } }
                                  );
                                  toast.success('Future dates removed successfully');
                                } else {
                                  // Delete entire schedule (all past, all future, or mixed)
                                  await axios.delete(
                                    `${backendUrl}/api/operator/schedules/${deleteConfirmSchedule._id}`,
                                    { headers: { Authorization: `Bearer ${operatorData?.token}` } }
                                  );
