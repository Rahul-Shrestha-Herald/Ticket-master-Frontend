...
const ManageRoutes = () => {
  ...
  // State for stop fares modal
  const [stopFaresModalOpen, setStopFaresModalOpen] = useState(false);
  const [stopFaresData, setStopFaresData] = useState([]); // [{ stop, fare }]
  ...
  const openModalForEdit = (route) => {
    setFormData({
      ...
      showPrice: route.showPrice !== undefined ? route.showPrice : true
    });
  ...
  const handleSubmit = async (e) => {
    ...
    if (Number(formData.price) < 0) {
      toast.error('Price cannot be negative. Use 0 if fare is calculated by stops.');
      return;
    }
  ...
  // ---------- STOP FARES FUNCTIONS ----------

  // Open stop fares modal — pre-populate from existing stopFares or build from route stops
  const openStopFaresModal = (route) => {
    // Build ordered stop list: from → pickupPoints → dropPoints → to
    const allStops = [route.from, ...route.pickupPoints, ...route.dropPoints, route.to];
    // Remove duplicates while preserving order
    const uniqueStops = allStops.filter((s, i) => allStops.indexOf(s) === i);

    if (route.stopFares && route.stopFares.length > 0) {
      // Use existing stopFares, fill in any missing stops
      const existing = [...route.stopFares];
      const data = uniqueStops.map(stop => {
        const found = existing.find(sf => sf.stop === stop);
        return { stop, fare: found ? found.fare : 0 };
      });
      setStopFaresData(data);
    } else {
      setStopFaresData(uniqueStops.map((stop, i) => ({ stop, fare: i === 0 ? 0 : '' })));
    }
    setSelectedRoute(route);
    setStopFaresModalOpen(true);
  };

  const handleStopFareChange = (index, value) => {
    const updated = [...stopFaresData];
    updated[index].fare = value;
    setStopFaresData(updated);
  };

  const handleStopFaresSubmit = async (e) => {
    e.preventDefault();
    // Validate: all fares except first must be > 0
    for (let i = 1; i < stopFaresData.length; i++) {
      if (!stopFaresData[i].fare || Number(stopFaresData[i].fare) <= 0) {
        toast.error(`Please enter a valid fare for stop: ${stopFaresData[i].stop}`);
        return;
      }
    }
    try {
      await axios.put(
        `${backendUrl}/api/operator/routes/stop-fares/${selectedRoute._id}`,
        { stopFares: stopFaresData.map(sf => ({ stop: sf.stop, fare: Number(sf.fare) })) },
        { headers: { Authorization: `Bearer ${operatorData?.token}` } }
      );
      toast.success('Stop fares saved successfully');
      setStopFaresModalOpen(false);
      fetchRoutes();
    } catch (error) {
      toast.error('Failed to save stop fares');
    }
  };
    ...
    <OperatorLayout>
      <div className="p-2 sm:p-4 md:p-6">
        <div className="max-w-8xl mx-auto p-2 sm:p-4 md:p-8">
                  ...
                  >
                    ...
                    <div className="mt-6 sm:mt-8 flex flex-wrap gap-2 sm:gap-4">
                      ...
                      <Button
                        ...
                        color="success"
                        startIcon={<FaPlusCircle />}
                        onClick={() => openStopFaresModal(route)}
                        className="text-xs sm:text-sm"
                      >
                        Stop Fares
                      </Button>
          ...
          <Modal open={modalOpen} onClose={closeModal}>
            <Box sx={modalStyle}>
              ...
              <form onSubmit={handleSubmit} className="space-y-3 sm:space-y-4">
                ...
                />
                {/* Show Price Toggle */}
                <Box className="flex items-center justify-between p-3 border border-gray-200 rounded-lg bg-gray-50">
                  <div>
                    <Typography variant="body2" className="font-medium text-gray-700">Show price on ticket listing</Typography>
                    <Typography variant="caption" className="text-gray-500">
                      If off, passengers will see "Price varies by stop" instead of the base price
                    </Typography>
                  </div>
          ...
          {/* Modal for Stop Fares */}
          <Modal open={stopFaresModalOpen} onClose={() => setStopFaresModalOpen(false)}>
            <Box sx={modalStyle}>
              <div className="flex justify-between items-center mb-4">
                <Typography variant="h5">Set Stop Fares</Typography>
                <IconButton onClick={() => setStopFaresModalOpen(false)}>
                  <FaTimes className="text-red-600" />
                </IconButton>
              </div>
              ...
              {selectedRoute && (
                <>
                  <div className="bg-blue-50 border border-blue-200 rounded p-3 mb-4 text-sm text-blue-700">
                    <strong>How it works:</strong> Enter the fare for each segment (cost to travel from the previous stop to this stop). The system will automatically sum the fares between the passenger's boarding and destination stops.
                  </div>

                  <Typography variant="subtitle2" className="mb-3">
                    Route: <strong>{selectedRoute.from}</strong> → <strong>{selectedRoute.to}</strong>
                  </Typography>

                  <form onSubmit={handleStopFaresSubmit}>
                    <div className="max-h-[55vh] overflow-y-auto pr-1 space-y-3">
                      {stopFaresData.map((sf, index) => (
                        <Box key={index} className="flex items-center gap-3 p-2 border rounded-lg bg-gray-50">
                          <div className="flex items-center gap-2 flex-1">
                            <div className={`w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold text-white shrink-0 ${index === 0 ? 'bg-green-500' : index === stopFaresData.length - 1 ? 'bg-red-500' : 'bg-blue-500'}`}>
                              {index + 1}
                            </div>
                            <Typography className="text-sm font-medium">{sf.stop}</Typography>
                          </div>
                          {index === 0 ? (
                            <Typography className="text-xs text-gray-400 italic w-32 text-right">Boarding stop (Rs. 0)</Typography>
                          ) : (
                            <TextField
                              label="Fare from prev stop"
                              type="number"
                              size="small"
                              value={sf.fare}
                    ...
                    <Box className="flex justify-end gap-3 mt-4">
                      <Button variant="outlined" onClick={() => setStopFaresModalOpen(false)}>Cancel</Button>
                      <Button variant="contained" color="success" type="submit">Save Stop Fares</Button>
                    </Box>
                  </form>
                </>
              )}
