...
import KYCManagementView from '../../../components/adminrenderview/kycmanagement/KYCManagementView';

const AdminDashboard = () => {
  const navigate = useNavigate();
  const { backendUrl, setIsAdminLoggedin, adminData, setAdminData, getAdminData, setSuppressUnauthorizedToast } = useContext(AdminAppContext);
  ...
  const handleProfilePictureChange = async (e) => {
    ...
    try {
      ...
      const { data } = await axios.post(`${backendUrl}/api/admin/profile-picture`, formData, {
  ...
  useEffect(() => {
    ...
    if (['users', 'buses', 'routes', 'support', 'kyc'].includes(activeView)) {
      fetchData();
    }
  ...
  const fetchKYCSubmissions = async () => {
    try {
      const response = await axios.get(`${backendUrl}/api/admin/kyc`, {
        params: { status: statusFilter === 'all' ? null : statusFilter },
        headers: { Authorization: `Bearer ${adminData?.token}` }
      });
      if (response.data.success) {
        setKycSubmissions(response.data.kycs || []);
      }
    } catch (error) {
  ...
  const handleApproveKYC = async (kycId) => {
    try {
      const response = await axios.post(
        `${backendUrl}/api/admin/kyc/${kycId}/approve`,
        {},
        { headers: { Authorization: `Bearer ${adminData?.token}` } }
      );
      if (response.data.success) {
  ...
  // Handle Reject KYC
  const handleRejectKYC = async (kycId, rejectionReason) => {
    try {
      const response = await axios.post(
        `${backendUrl}/api/admin/kyc/${kycId}/reject`,
        { rejectionReason },
        { headers: { Authorization: `Bearer ${adminData?.token}` } }
      );
      if (response.data.success) {
  ...
  const navigationItems = [
    ...
    { name: 'KYC Verification', icon: FaIdCard, view: 'kyc' },
      ...
      case 'kyc':
        return (
          <KYCManagementView
            ...
            onSearchChange={(e) => setSearchQuery(e.target.value)}
            onFilterChange={(e) => setStatusFilter(e.target.value)}
            onViewDetails={(kyc) => setSelectedKYC(kyc)}
            onApprove={handleApproveKYC}
            onReject={handleRejectKYC}
            selectedKYC={selectedKYC}
            onCloseDetails={() => setSelectedKYC(null)}
            fetchKYCs={fetchKYCSubmissions}
          />
        );
          ...
          <div className="flex items-center mt-2 gap-3">
            <div className="relative">
              ...
              <button
                ...
                className="absolute bottom-0 right-0 w-5 h-5 bg-blue-600 rounded-full flex items-center justify-center text-white shadow hover:bg-blue-700 transition-colors disabled:opacity-60"
            ...
            <p className="text-sm text-gray-600">Welcome, {adminData?.name}</p>
          </div>
        </div>
