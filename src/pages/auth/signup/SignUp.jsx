...
const Signup = () => {
    ...
    const handleNameChange = (e) => {
        ...
        if (/\d/.test(val)) {
            ...
            setNameError('Name must be in lowercase letters only.');
        } else if (/\s{2,}/.test(val) || val.startsWith(' ')) {
            setNameError('Name cannot start with or have multiple spaces.');
    ...
    const validateForm = () => {
        ...
        if (/[A-Z]/.test(name)) {
            toast.error("Name must be in lowercase letters only.");
            return false;
        }
        if (/[^a-z\s]/.test(name)) {
                ...
                <form onSubmit={onSubmitHandler} noValidate>
                    <div className='mb-4 relative flex items-center gap-3 w-full px-5 py-2.5 rounded-full bg-[#333A5C]'>
                        ...
                        <input
                            ...
                            type="text"
                            placeholder='Full Name (lowercase only)'
                            required
                        />
                        {nameError && (
                            <span className="absolute -bottom-5 left-4 text-red-400 text-[11px] whitespace-nowrap">
                                {nameError}
                            </span>
                        )}
                    </div>
