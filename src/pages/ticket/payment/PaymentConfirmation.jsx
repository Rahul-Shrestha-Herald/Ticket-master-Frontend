...
const PaymentConfirmation = () => {
    ...
    // Fetch seat labels on mount
    useEffect(() => {
        const fetchSeatLabels = async () => {
            if (!ticketDetails?.busId || !ticketDetails?.selectedSeats || ticketDetails.selectedSeats.length === 0) {
                return;
            }
            ...
            try {
                const response = await axios.get(`${backendUrl}/api/bus/seat-display-data`, {
                    params: { 
                        ...
                        seatIds: JSON.stringify(ticketDetails.selectedSeats) 
                    }
                });

                if (response.data.success && response.data.data.seatDisplayData) {
                    setSeatLabels(response.data.data.seatDisplayData);
                } else {
                    // Fallback to using seat IDs
                    setSeatLabels(ticketDetails.selectedSeats);
                }
            } catch (error) {
        ...
        fetchSeatLabels();
    }, [ticketDetails, backendUrl]);

    // Fetch user's TM Points balance
    useEffect(() => {
        const fetchTMPoints = async () => {
            try {
                const { data } = await axios.get(`${backendUrl}/api/user/tm-points`);
                if (data.success) setTmPoints(data.tmPoints);
            } catch (_) { /* non-fatal */ }
        };
        fetchTMPoints();
    }, [backendUrl]);
    ...
    useEffect(() => {
        ...
        // NOTE: We no longer auto-initiate — payment is initiated on Pay Now click
        // so that TM Points discount is included in the amount
    ...
    const handleApplyPoints = async () => {
        ...
        try {
            const { data } = await axios.post(`${backendUrl}/api/user/tm-points/validate`, {
                pointsToRedeem,
                transactionAmount: ticketDetails?.totalPrice || 0
            });
            if (data.success) {
                // Use the server-returned rounded value (multiples of 100)
                setPointsToRedeem(data.pointsToRedeem);
                setPointsDiscount(data.discountAmount);
                // Reset payment URL so it gets re-initiated with the new discounted amount
                setPaymentUrl('');
                paymentInitiatedRef.current = false;
                localStorage.removeItem('paymentUrl');
                localStorage.removeItem('paymentData');
                toast.success(`Rs. ${data.discountAmount} discount applied!`);
            } else {
                toast.error(data.message || 'Cannot apply points');
                setPointsDiscount(0);
            }
        } catch (err) {
            toast.error(err.response?.data?.message || 'Failed to apply points');
            setPointsDiscount(0);
        } finally {
    ...
    const handleRemovePoints = () => {
        setPointsToRedeem(0);
        setPointsDiscount(0);
        // Reset payment URL so it gets re-initiated with the original amount
        setPaymentUrl('');
        paymentInitiatedRef.current = false;
        localStorage.removeItem('paymentUrl');
        localStorage.removeItem('paymentData');
    ...
    const initiatePayment = async () => {
        try {
            ...
            const paymentData = {
                amount: Math.max(ticketDetails.totalPrice - pointsDiscount, 0),
                reservationId: reservation.id,
                passengerInfo: {
                ...
                pointsToRedeem: pointsDiscount > 0 ? pointsToRedeem : 0,
            };
    ...
    const handlePayNow = async () => {
        ...
        // If payment URL already exists (initiated with same discount), go directly
        if (paymentUrl) {
            setLoading(true);
        ...
        // Initiate payment now with current discount applied
            ...
            <RootLayout className="space-y-8 w-full pb-16">
                <div className="max-w-2xl mx-auto bg-white rounded-2xl shadow-md p-4 md:p-6 lg:p-8">
                    <div className="space-y-4 md:space-y-6">
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 md:gap-4">
                                ...
                                <div>
                                    ...
                                    <p className="text-sm md:text-base font-medium">
                                        {seatLabels.length > 0 ? seatLabels.join(', ') : (Array.isArray(ticketDetails?.selectedSeats) ? ticketDetails.selectedSeats.join(', ') : ticketDetails?.selectedSeats)}
                                    </p>
                                </div>
                            </div>
                        </div>
                        ...
                        {/* Price Summary */}
                        <div className="bg-neutral-50 p-3 md:p-4 rounded-lg border-t border-b border-neutral-200 space-y-2">
                            <div className="flex items-center justify-between text-sm text-neutral-600">
                                <p>Ticket Price</p>
                                <p>NPR {ticketDetails?.totalPrice}</p>
                            </div>
                            {pointsDiscount > 0 && (
                                <div className="flex items-center justify-between text-sm text-green-600">
                                    <p>TM Points Discount ({pointsToRedeem} pts)</p>
                                    <p>- NPR {pointsDiscount}</p>
                                </div>
                            )}
                            <div className="flex items-center justify-between font-bold border-t border-neutral-200 pt-2">
                                <p className="text-base md:text-lg">Total Amount</p>
                                <p className="text-lg md:text-xl text-primary">
                                    NPR {Math.max((ticketDetails?.totalPrice || 0) - pointsDiscount, 0)}
                                </p>
                            </div>
                        </div>

                        {/* TM Points Redemption */}
                        {tmPoints > 0 && (
                            <div className="bg-amber-50 border border-amber-200 rounded-xl p-4 space-y-3">
                                <div className="flex items-center gap-2">
                                    <FaCoins className="text-amber-500" />
                                    <p className="font-semibold text-neutral-800 text-sm">Use TM Points</p>
                                    <span className="ml-auto text-xs bg-amber-100 text-amber-700 px-2 py-0.5 rounded-full font-medium">
                                        {tmPoints} pts available
                                    </span>
                                </div>

                                {(ticketDetails?.totalPrice || 0) < 400 ? (
                                    <p className="text-xs text-neutral-500 bg-neutral-100 rounded-lg px-3 py-2">
                                        TM Points can only be used on transactions of <strong>NPR 400 or more</strong>. Your current total is NPR {ticketDetails?.totalPrice || 0}.
                                    </p>
                                ) : pointsDiscount > 0 ? (
                                    <div className="flex items-center justify-between bg-green-50 border border-green-200 rounded-lg px-3 py-2">
                                        <p className="text-sm text-green-700 font-medium">
                                            {pointsToRedeem} pts applied → Rs. {pointsDiscount} off
                                        </p>
                                        <button onClick={handleRemovePoints} className="text-xs text-red-500 hover:underline">Remove</button>
                                    </div>
                                ) : (
                                    <div className="flex gap-2">
                                        <input
                                            ...
                                            placeholder={`Enter points (max ${tmPoints})`}
                                            className="flex-1 border border-neutral-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary/30"
                                        />
                                        <button
                                            onClick={handleApplyPoints}
                                            disabled={applyingPoints || pointsToRedeem < 1 || pointsToRedeem > tmPoints}
                                            className="px-4 py-2 bg-primary text-white text-sm font-semibold rounded-lg hover:bg-primary/90 disabled:bg-neutral-300 transition-colors"
                                        >
                                            {applyingPoints ? '...' : 'Apply'}
                                        </button>
                                    </div>
                                )}
                                <p className="text-xs text-neutral-500">100 TM Points = Rs. 10 discount. Minimum transaction: NPR 400.</p>
                            </div>
                        )}

                        {/* Earn points notice */}
                        <div className="flex items-center gap-2 text-xs text-neutral-500 bg-neutral-50 rounded-lg px-3 py-2">
                            <FaCoins className="text-amber-400 shrink-0" />
                            <span>
                                You'll earn <strong className="text-neutral-700">
                                    {Math.floor(Math.max((ticketDetails?.totalPrice || 0) - pointsDiscount, 0) * 0.05)} TM Points
                                </strong> after this booking
                            </span>
                        </div>
